...
(head -2 "$f1" && awk -F"\t" '$8 == '$rep_index' { print;}' "$f1") > "$f11"_rep"${rep[$i]}".csv
(head -2 "$f11"_rep"${rep[$i]}".csv && tail -n +3  "$f11"_rep${rep[$i]}.csv | sort -nk7) > "$f11"_rep"${rep[$i]}"_sorted.csv
...
uentn=`tail -n +3 "$f11"_rep"${rep[$i]}"_sorted.csv | cut -f7 | sort -u | wc -l`
entn=`tail -n +3 "$f11"_rep"${rep[$i]}"_sorted.csv | cut -f7 | sort | wc -l`
...
(head -2 "$f11"_rep"${rep[$i]}"_sorted_nl.csv && sed -n '3~2p' "$f11"_rep"${rep[$i]}"_sorted_nl.csv) | cut -f 2- > "$f11"_rep"${rep[$i]}"_sorted_nl_1.csv

(head -2 "$f11"_rep"${rep[$i]}"_sorted_nl.csv && sed -n '4~2p' "$f11"_rep"${rep[$i]}"_sorted_nl.csv) | cut -f 2- > "$f11"_rep"${rep[$i]}"_sorted_nl_2.csv
