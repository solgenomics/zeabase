# ~/code/code_bash_shell/Check_Pedigree_Balance.sh file_pedigree_unique.txt
# ~/code/code_bash_shell/Check_Pedigree_Balance.sh pheno_154trials_backup_pedigree_line_tester.csv

while read line; do

pedigree=`echo "$line" | awk -F"," '{print $58}'`

Output=`./a.out "$pedigree"`

if [[ "$Output" == 'Not Balanced' ]] 
then 

echo "$pedigree"

#pedigree1=`echo "$pedigree" | sed 's/\\[/\\\[/g'`
#pedigree2=`echo "$pedigree1" | sed 's/\\]/\\\]/g'`
#pedigree3=`echo "$pedigree2" | sed 's/\\*/\\\*/g'`

#echo "$pedigree3"

#grep -n "$pedigree" pheno_154trials_backup_pedigree_line_tester.csv
#grep -n "$pedigree3" pheno_154trials_backup_pedigree_line_tester.csv
fi
