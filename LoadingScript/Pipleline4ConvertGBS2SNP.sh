#Usage:  bash ~/code/code_bash_shell/Pipleline4ConvertGBS2SNP.sh WEMA_population.txt 
...
file1=`echo "$path3" | sed -e 's/ /\\\ /g' | sed -n 2,2p`
...
pop_name=`echo "$path3" | awk -F"." '{print $1}' | sed 's/ /_/g' |sed -n 2,2p`
echo "$pop_name" #5
