# ~/code/code_bash_shell/Replace_EXP_name.sh makumbi_pheno_154trials_backup_updated1.csv
...
if echo "$line" | grep -qi -e "S2-LINE TRIAL-1-1"
then 
echo "$line" | awk -v OFS="," -F"," '{$4="S2-LINE-TRIAL-1-1";print;}'
else
echo "$line" | awk -v OFS="," -F"," '{print;}'
fi
