# ~/code/code_bash_shell/Partition_pedigree_into_line_tester_for_each_trial2.sh trail_name_project_line_tester_pedigree_no_title2.txt

while read line; do

trial_name=`echo "$line" | cut -f1`
pedigree=`echo "$line" |cut -f5`

Pedi_line=`echo "$line" |cut -f3`
Pedi_tester=`echo "$line" |cut -f4`
