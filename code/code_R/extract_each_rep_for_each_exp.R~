...
data.trait_exp<-data.for.read[which(data.for.read[,2]==rep[i]),6:10]

data.rep.output<-cbind(ENTRY,REP_BLOCK_PLOT,DESIG,NOPLT,NOSV,data.trait_exp);


write.table(data.rep.output,file=paste(file.name,"rep-",rep[i],".csv",sep=""),append = F,na=" ",quote = F, sep = "\t",eol = "\n",row.names = F,col.names = F);
}
